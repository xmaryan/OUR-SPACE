...
export function AdminCrud({ title, subtitle, listEndpoint, createEndpoint, itemEndpoint, fields, columns, defaultValues = {} }: Props) {
  ...
  const load = async () => {
    setLoading(true);
    try { const r = await api.get(listEndpoint); setItems(r.data); }
    catch {} finally { setLoading(false); }
  };
  useEffect(() => { load(); }, [listEndpoint]);

  const openNew = () => { setEditing(null); setForm({ ...defaultValues }); setShowForm(true); setMsg(null); };
  const openEdit = (it: any) => { setEditing(it); setForm({ ...defaultValues, ...it }); setShowForm(true); setMsg(null); };

  const submit = async () => {
    for (const f of fields) {
      if (f.type !== "select" && !form[f.key] && f.key !== "recovery_email" && f.type !== "date") { /* allow empty */ }
    }
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.xl }}>
        ...
        {loading ? (
          ...
          <View style={styles.empty}>
            ...
            <Text style={{ color: colors.muted }}>No items yet. Click "Add new" to create one.</Text>
          </View>
        ) : (
...
const styles = StyleSheet.create({
  ...
  modalOverlay: { position: "absolute", inset: 0 as any, top: 0, left: 0, right: 0, bottom: 0, backgroundColor: "rgba(0,0,0,0.4)", alignItems: "center", justifyContent: "center", padding: spacing.lg },
